...
class QAgent:
    def __init__(self, state_space, action_space, init_val=0):
        ...
        """
        self.s_space = state_space.n
        self.a_space = action_space.n
        self.Q_value_table = np.zeros((self.s_space, self.a_space)) + init_val
    ...
    def _act_train(self, state, epsilon, action_mask=None):
        ...
        if np.random.uniform(0, 1, 1) < epsilon:
            action = random.randint(0, self.a_space-1)
    ...
    def update(self, state, action, reward, next_state, alpha, gamma, next_state_action_mask=None):
        ...
        """
        next_action = np.argmax(self.Q_value_table[next_state])
        self.Q_value_table[state, action] = (1 - alpha) * self.Q_value_table[state, action] + alpha * (reward + gamma * self.Q_value_table[next_state, next_action])
